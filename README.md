...
# 진행상황 240725
- EC2에서 서버 실행 후 내 IP에서 클라이언트 연결함
  - 보안그룹 인바운드 규칙에 8484번 포트 추가

# 진행상황 240730
- 최적화를 위해 C로 웹훅 서버를 작성 (실제 성능 차이가 있는지 확인해보긴 해야함)

# 진행상황 240812
- 인스턴스에서 ssh-keygen 이용 ssh 키 발급하여 깃헙 계정에 등록
- 원격 저장소 셋업
- CosmicWebhook 첫번째 커밋 push

# 해볼만한 것들
- 빌드와 실행을 동시에 할 수 있도록 하는 `startup_script.sh` 작성
- 해당 스크립트가 인스턴스가 시작될 때 자동으로 실행되도록 서비스 추가
- CosmicWebhook 깃헙 연동
- 환경변수 설정으로 서버 인스턴스가 스스로가 어떤 환경에서 실행되는지 알게 하기 - 실행 환경에 따라 다른 환경변수 설정
- http / https (보안)
- 서버 인스턴스 이미지 전체 도커로 말기
